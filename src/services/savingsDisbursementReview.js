...
export const loadSavingsConsistencyAssessment = async ({ api, loan, referenceDate }) => {
  if (!loan?.member) return { member: null, group: null, assessment: { applicable: false } };

  const member = await api.collection('members').getOne(loan.member, { expand: 'group' });
  if (['suspended', 'closed', 'exited'].includes(String(member.status || '').toLowerCase())) {
    throw new Error('Cannot process a loan for a suspended or closed member.');
  }
  if (!member.group) return { member, group: null, assessment: { applicable: false } };
  ...
  const savings = await api.collection('savings').getFullList({
    ...
    fields: 'id,member,type,amount,date,created,is_reversed',
    sort: 'date,created'
  });
  const assessment = calculateSavingsConsistency({ member, group, savings, referenceDate });
  return { member, group, assessment };
};
...
export const reviewSavingsBeforeDisbursement = async ({ api, loan, disbursementDate, getUser, confirmException, now = new Date() }) => {
  ...
  const loadAssessment = async () => {
    const { member, assessment } = await loadSavingsConsistencyAssessment({ api, loan, referenceDate: disbursementDate });
